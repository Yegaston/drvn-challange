...
use App\Models\Shared\CommonInternalResponse;
...
trait HttpResponse
{
    ...
    protected function respondWithCommonResponse(CommonInternalResponse $response)
    {
        if ($response->error) {
            ...
            return $this->setStatusCode($response->status)
                ->respondWithSuccess($response->message, $response->data);
        }
    }
    ...
    protected function response($data, array $headers = [])
    {
        ...
        if ($data instanceof Arrayable && !$data instanceof JsonSerializable) {
            ...
            $data = $data->toArray();
        } elseif ($data instanceof \App\Http\Resources\Responses\SuccessResource) {

        } elseif ($data instanceof JsonResource && !$this->hasErrors()) {
            return $data->toResponse(request())->setStatusCode($this->getStatusCode());
        }
        elseif ($data->resource instanceof \Symfony\Component\HttpKernel\Exception\AccessDeniedHttpException) {
            $data = [
                'error' => true,
                'errors' => [],
                ...
                'exception' => null,
                'httpCode' => 403,
                'message' => 'El usuario no tiene acceso a los recursos solicitados.'
            ];
        }
